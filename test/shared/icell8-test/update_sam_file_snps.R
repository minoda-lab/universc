...
#   print(table( (vcf_pos >= sam_pos) & (vcf_pos <= sam_pos+read_length)  ))
   matched_reads <- ((sam_chr ==  vcf_chr) & (vcf_pos >= sam_pos) & (vcf_pos <= sam_pos+read_length) & (!is.null(ref_len)))
#if(any(matched_reads)){
...
#   print(sam_qual[matched_reads])
 ...
 #  print( substr(sam_seq[matched_reads], vcf_pos - sam_pos[matched_reads], vcf_pos - sam_pos[matched_reads] + alt_len) )
 ...
 print ( vcf_ref )
   ...
   #                     collapse = "")
   #              }
     print(     substr(sam_seq[matched_reads], vcf_pos - sam_pos[matched_reads] + 1, vcf_pos - sam_pos[matched_reads] + ref_len)   )
#   print(sam_seq[matched_reads][vcf_pos - sam_pos[matched_reads] + 1])
